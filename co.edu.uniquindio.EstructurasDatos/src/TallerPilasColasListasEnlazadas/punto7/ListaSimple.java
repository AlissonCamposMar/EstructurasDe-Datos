package TallerPilasColasListasEnlazadas.punto7;
...
//				nodoUltimo = null;
	...
	/**
	 * Funcion recursiva que invierte la lista simple generica
	 * @param lista
	 * @param nodo
	 * @return
	 */
	public ListaSimple<Integer> invertirLista(ListaSimple<Integer> lista, Nodo nodo) {
		try {
			if (!nodo.equals(nodoUltimo)) {
				Nodo nodoAux = nodo.getSiguienteNodo();
				lista.eliminar((Integer) nodoAux.getValorNodo());
				lista.agregarInicio((Integer) nodoAux.getValorNodo());
				invertirLista(lista, nodo);
			}
			return lista;
		} catch (Exception e) {
			return lista;
