...
public class App {
	...
	/*
	 * Metodo que agrega un proceso al vector de ProcesoGUI
	 */
	private void llenarProcesos() {
		...
		/*
		 * isEmpty()Prueba si este vector no tiene componentes. 
		 * return true si y solo si este vector no tiene componentes, es decir,
		 * su tamaño es cero; false de lo contrario.
		 */
		llenarActividades(procesos.isEmpty() ? null : procesos.get(0));
	}

	/*
	 * Metodo que agrega una actividad al vector de ActividadGUI
	 */
	...
	public static GestorDeProcesos crearSetPruebas()
		...
		/*
		 * Se crean los procesos Desayunos Sopresa y Emsabladora de Automï¿½viles
		 */
		...
		gestor.obtenerProceso("p2").crearActividadAlFinal("Pintura", "Pintar las piezas de la carrocerï¿½a", true);
		...
		/**
		 ******* CREAR TAREAS**** *** PROCESO ENSAMBLADORA DE AUTOMï¿½VILES***** Se crean las
		...
		/*
		 * Se crean las tareas para la actividad Carrocerï¿½a
		 */
		...
		gestor.obtenerProceso("p2").buscarActividadNombre("Pintura").crearTareaAlFinal("Pintar toda la carrocerï¿½a",
				true, 3);
