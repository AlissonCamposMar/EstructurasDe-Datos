...
public class CrearNuevaTareaOpcionalTest {
	...
	public void test() throws ActividadNoExisteException, ValidacionTareaOpcionalException {
		...
		String nombresEsperados[] = {"Lavar la fruta","Quitar semillas","Cortar la fruta","Adicionar agua o leche","Adicionar azúcar", "Licuar la fruta"}; 
		int i = 0;
		for (Tarea tarea : p1) {
			Assert.assertEquals(nombresEsperados[i], tarea.getDescripcion());
			i++;
		}
		Tarea tarea = gestor.obtenerProceso("p1").buscarTareaEnActividadNombre("Preparar jugo", "Quitar semillas");
		Assert.assertEquals("Quitar semillas", tarea.getDescripcion());
