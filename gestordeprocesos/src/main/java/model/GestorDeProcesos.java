...
public class GestorDeProcesos implements Iterable<Proceso>{
	 ...
	 * Metodo crearProceso
	 *@Param  id,nombre
	 *Metodo que permite crear un nuevo proseco validando por medio del id que el proceso  no exista  
	 */
	...
	public Cola <ProcesoActividad> buscarActividad (String nombreActividad){
		...
		for (Proceso proceso : procesos) {
			...
			if(actividad!=null) {
				listaAuxiliar.add(new ProcesoActividad(proceso, actividad));
			}
		}
